...
with lib; let
  ...
  mkBrewfileSectionString = heading: entries:
  ...
  mkBrewfileLineValueString = v:
    ...
    then strings.floatToString v
    else if isBool v
    then boolToString v
    else if isString v
    then ''"${v}"''
    else if isAttrs v
    then "{ ${concatStringsSep ", " (mapAttrsToList (n: v': "${n}: ${mkBrewfileLineValueString v'}") v)} }"
    else if isList v
    then "[${concatMapStringsSep ", " mkBrewfileLineValueString v}]"
  ...
  mkDocOptionLink = optionName: ''<link xlink:href="#opt-${optionName}"><option>${optionName}</option></link>'';

  mkNullOrBoolOption = args:
    mkOption (args
      // {
  ...
  onActivationOptions = {config, ...}: {
    options = {
      ...
      silent = mkOption {
        type = types.bool;
        default = false;
        description = "Make brew output more quiet.";
      };
  ...
  globalOptions = {config, ...}: {
    ...
    config = {
      homebrewEnvironmentVariables = {
        ...
        HOMEBREW_NO_AUTO_UPDATE = mkIf (!cfg.onActivation.autoUpdate) "1";
  ...
  brewOptions = {config, ...}: {
    ...
    in {
      brewfileLine =
        ...
        # We need to handle the `restart_service` option separately since it can be either a bool
        # or `:changed` in the Brewfile.
        + optionalString (sCfg ? restart_service) (
          ", restart_service: "
          + (
            if isBool config.restart_service
            then sCfg.restart_service
